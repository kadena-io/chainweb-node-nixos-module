...
in
{
  ...
  options = {
    services.chainweb-node = {
      ...
      # let syncFlag = if cfg.replay then "--delete" else "--update"
      # ${pkgs.rsync}/bin/rsync -avz --progress ${syncFlag} --checksum ${bootstrapNode}::db/0/rocksDb
      ...
      replay = mkEnableOption (lib.mdDoc ''
        Run a replay, disregarding almost all customisation.
      '');

      continueAfterReplay = mkEnableOption (lib.mdDoc ''
        After a replay, switch to running the node regularly.
      '');
